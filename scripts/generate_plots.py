...
def load_detection_results():
    ...
    with open(path) as f:
        raw = json.load(f)
    eps_keys = {"0.1", "0.5", "1.0", "5.0", "inf"}
    return {k: v for k, v in raw.items() if k in eps_keys}
...
def plot_privacy_utility(results):
    ...
    f1_attack = [results.get(str(e), {}).get("f1_attack",   0) for e in epsilons]
    f1_benign = [results.get(str(e), {}).get("f1_benign",   0) for e in epsilons]
    auroc     = [results.get(str(e), {}).get("auroc",       0) for e in epsilons]

    no_dp = results.get("inf", {})
    if no_dp:
        f1_attack.append(no_dp.get("f1_attack", 0))
        f1_benign.append(no_dp.get("f1_benign", 0))
        auroc.append(no_dp.get("auroc", 0))
        eps_labels.append("∞\n(no DP)")

    x = list(range(len(eps_labels)))

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(x, f1_attack, 'o-',  color=COLORS["quarantined"],
            linewidth=2, markersize=7, label="Attack F1")
    ax.plot(x, f1_benign, 's-',  color=COLORS["trusted"],
            linewidth=2, markersize=7, label="Benign F1")
    ax.plot(x, auroc,     '^--', color=COLORS["purple"],
            linewidth=1.5, markersize=6, label="AUROC")

    ax.axhline(0.5, color=COLORS["gray"], linewidth=0.8, linestyle=":",
    ...
    if len(x) >= 2:
        ax.axvspan(-0.5, 1.5, alpha=0.04, color=COLORS["trusted"])
        ax.axvspan(1.5, len(x) - 0.5, alpha=0.04, color=COLORS["suspicious"])
...
def plot_precision_recall(results):
    ...
    prec_attack = [results.get(str(e), {}).get("precision_attack", 0) for e in epsilons]
    rec_attack  = [results.get(str(e), {}).get("recall_attack",    0) for e in epsilons]
    ...
    ax.set_ylim(0, 1.1)
    ax.set_title(
        "Figure 2: Attack Detection Precision and Recall\nacross Privacy Budgets",
        fontsize=12,
    )
    ax.legend()
    ax.grid(axis="y", alpha=0.3)
...
def plot_fairness(fairness):
    ...
    if not agents:
        print("[plots] No per-agent fairness data found, skipping Figure 3.")
        return

    fprs = [fairness[a].get("false_positive_rate", 0) for a in agents]
    tprs = [fairness[a].get("true_positive_rate",  0) for a in agents]

    x = np.arange(len(agents))
    w = 0.35

    fig, ax = plt.subplots(figsize=(8, 5))
    bars1 = ax.bar(x - w/2, fprs, w,
                   label="False Positive Rate (benign flagged)",
                   color=COLORS["suspicious"])
    ax.bar(x + w/2, tprs, w,
           label="True Positive Rate (attacks caught)",
    ...
    ax.set_ylim(0, 1.1)
    ax.set_title(
        "Figure 3: Fairness Analysis — Per-Agent False Positive and True Positive Rates\n"
        "(ε=1.0,  lower FPR = more fair)",
        fontsize=12,
    )
...
def plot_trust_timeline():
    ...
    for i in range(beats):
        div_exec = (
            np.random.uniform(18, 22)   if i < 25 else   
            np.random.uniform(150, 350) if i < 40 else   
    ...
    ax.set_xlim(0, 68)
    ax.set_title(
        "Figure 4: Trust Score Timeline — Prompt Injection Attack on Executor\n"
        "Only the attacked agent degrades; Planner remains trusted throughout",
        fontsize=12,
    )
    ax.legend(loc="upper right")
    ax.grid(alpha=0.2)
...
if __name__ == "__main__":
    ...
    if det:
        ...
        print("[plots] Run evaluate_detectors.py first.")
